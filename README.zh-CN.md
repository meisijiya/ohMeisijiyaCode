...
## 🚀 异步委派（opencode 1.16.2+）

Sisyphus、Lyra、Hephaestus 可以用 `task-dispatch` 工具的 3 种 `mode` 值，**异步**地把工作分派给子 agent。需要环境变量 `OPENCODE_EXPERIMENTAL=true`（或 `..._BACKGROUND_SUBAGENTS=true`）。

### 4 种模式

| `mode` | 行为 | 适用场景 |
|--------|------|----------|
| `background` (默认) | Fire-and-forget；子 agent 并行跑；**结果自动 inject** 到父 session | 独立任务、Sisyphus 期间有别的事、当前不需要这个结果再继续 |
| `sync` | 阻塞等子 agent 跑完；结果内联返回 | 短任务、Sisyphus 当前空闲、需要结果才能做下一步决策 |
| `continuation` | 通过 `task_id` 续接之前的子 agent session（保留对话历史）| 多轮协作（中途加约束、改方向） |

**Fan-out**（多个子 agent 真并行）：在同一个 response 里循环调 N 次 `task-dispatch`，每次 `mode=background` — opencode 让它们真正并行启动。

### 5 个真实场景

**场景 1：并行调研** — 同时比较 3 个候选库
```typescript
// Sisyphus prompt
...
// 3 个 Lyra session 真并行；Sisyphus 等 3 个全 inject 后综合分析
```

**场景 2：Bug 诊断（长）** — 诊断 production 崩溃
```typescript
// 第 1 轮：启动诊断
task-dispatch({ subagent_type: "lyra", mode: "background",
  description: "diagnose prod crash",
  prompt: "用 diagnose skill 分析 production 崩溃..." })
// Sisyphus 继续做其他事；Lyra 的诊断稍后自动 inject

// 第 2 轮：在同一个 Lyra session 上追问
task-dispatch({ subagent_type: "lyra", mode: "continuation",
  task_id: "<prior_id>",
  description: "follow-up diagnosis",
  prompt: "基于上次的分析，再深挖 X 模块..." })
```

**场景 3：批量 boilerplate** — 为 10 个相似实体创建 CRUD 文件
```typescript
// 用 sync（不是 background）：串行避免写冲突
task-dispatch({ subagent_type: "hephaestus", mode: "sync",
  description: "create 10 CRUD files",
  prompt: "为 User, Post, Comment, Tag, ... 创建 CRUD files..." })
// Sisyphus 阻塞等 10 个文件都写完，再 review
```

**场景 4：Fire-and-forget 长任务** — 跑 30 分钟的测试覆盖率分析
```typescript
// 启动后立刻去做别的事
task-dispatch({ subagent_type: "lyra", mode: "background",
  description: "analyze test coverage",
  prompt: "运行完整测试套件并分析覆盖率..." })
// Sisyphus 继续规划下一个 feature；30 min 后覆盖率报告自动 inject
```

**场景 5：实时多轮** — Sisyphus 和 Lyra 一起迭代 API 设计
```typescript
// 第 1 轮
task-dispatch({ subagent_type: "lyra", mode: "continuation",
  task_id: "<ses_id>",  // 复用 Lyra 的 session
  description: "refine API design",
  prompt: "加上 rate limiting 的考虑..." })
// Lyra 的 context 累积；两边都能看到完整的设计历史
```

### 为什么不直接用 opencode 的 `task` 工具？

`task` 是 opencode 内置工具。我们包成 `task-dispatch` 是因为：
1. 提供 **3 值 `mode` 枚举**（比同时管理 `background` + `task_id` 标志清晰）
2. 给出默认值（`mode=background`），让 Sisyphus"默认异步"
3. 把 MCP 代理调用统一在同一接口（`subagent_type="mcp:<server>:<tool>"`）
4. 跟 `Sisyphus` prompt 教的协议对齐 — 见 `agents/sisyphus.md` 的 `<delegation_protocol>` 段

### 硬约束

- **Fan-out 上限**：≤ 3 个并行子 agent（避免资源打满 + 上下文污染）
- **续接间隔**：同一个 task 30 秒内不要续接（浪费 token）
- **`OPENCODE_EXPERIMENTAL=true` 必须设**（系统 env / `.env` / shell init 任一）
...
| **[oh-my-openagent](https://github.com/code-yeongyu/oh-my-openagent)** | 架构灵感（orchestrator → specialists）| 不引入 100K+ LOC 代码库。opencode 1.16.2+ 原生支持后台子 agent（无需 HTTP 代理）|
...
| 洞察 | 应用 |
|------|------|
| **U 型注意力曲线**（整个 context 窗口 → 首+末都高注意力）| **修正**：U 型描述的是**整个 LLM context 窗口**（system prompt + 对话 + 工具结果），不是 agent 提示词内部结构。**System prompt 区域** = "首"高注意力区（含 3 agent 提示词）。**最近对话** = "末"高注意力区。**Context 中段** = 低注意力（早期对话被挤到中段）。HTML 末尾铁律在"首"区（稳定），不在"末"区 |
| **硬约束词汇**（never/always/must/绝对不要）| 重写 3 agent 的 `style_guide` 用强约束 + 反例/正例；末尾 HTML 块用 `**bold**` 加粗 + 括号内解释理由 |
| **Skill 文件 ≤ 300-500 行**| 全部 skill 都在 250 行内；**Sisyphus.md 现在 253 行**（从 363 行减下来——148 行硬编码协议被下沉到 superpowers skill 实时注入）|
| **方案 1+2+3+4**（AGENTS.md + Scan + Hooks + 子 Agent 隔离）| 全部有：orchestrator plugin (`experimental.chat.system.transform`) = Hook；子 agent 隔离 = 1+1+1 架构核心；**职责就近原则**（每个 agent 在自己 prompt 里用 `<responsibility_boundary>` 块声明——不依赖外部 AGENTS.md）|
| **反被动压缩**（不等质量下降再压缩）| **真实失败案例**：Sisyphus 363 行 prompt 变得难维护。Sisyphus 用了 `task-dispatch` 工具而非 `task` 工具——**不是因为"被 U 型注意力吞掉"**（协议在中段块中**是可见的**），而是**分层违规**（Sisyphus 自己决定用非标准工具，忽略了显式协议）。修复：把协议下沉到 superpowers skill（实时注入）是为了**架构清晰**，不是逃避 U 型注意力 |
| **软约束 = 没约束** | `bash: *: allow`（项目内信任） + 硬 deny 黑名单（不是"尽量"）|
| **职责就近原则**（新）| 每个 agent 的"我是谁"声明**活在它自己的 prompt 里**（`<responsibility_boundary>` 块）。外部文档引用有被 U 型注意力吞掉的风险——自带是最可靠的位置 |
| **机制层 vs 装饰层分层**（新）| Sisyphus prompt = **Static 层**（路由判断 / 风格 / 硬约束）+ **Dynamic 层**（协议由 superpowers skill 注入）。Static 层短而高信号，活在尾部 |
| **6 块结构对称**（新）| 3 个 agent prompt 共享结构：`role` / `responsibility_boundary` / `capabilities` / `style_guide` + 末尾 HTML 块。差异在**内容**（业务能力、工具列表、风格），不在**结构**。可用 grep 跨 agent 审计 |
